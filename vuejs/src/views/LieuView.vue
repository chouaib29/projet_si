...
export default {
  ...
  methods: {
    chargerLieux() {
      axios.get('http://localhost:8080/api/v1/lieux')
          .then(response => {
    ...
    modifierLieu(lieu) {
      // Logique pour modifier un lieu, potentiellement ouvrir un formulaire de modification
    },
    supprimerLieu(id) {
      axios.delete(`http://localhost:8080/api/v1/lieux/${id}`)
          .then(() => this.chargerLieux())
          .catch(error => console.error("Erreur lors de la suppression du lieu", error));
    },
  }
};
</script>
