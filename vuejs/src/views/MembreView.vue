    ...
    <div v-if="evenementsDuMembre.length > 0">
      ...
      <table>
        ...
        <tr v-for="evenement in evenementsDuMembre" :key="evenement.id">
          ...
          <td>
            <button
                @click="desinscrireMembre(evenement)"
                class="rouge"
            >
              Désinscrire
            </button>
  ...
  methods: {
    ...
    desinscrireMembre(evenement) {
      ...
      if (this.membreActuel && evenement && evenement.id_evnt) {
        ...
        axios
            .post(`/api/v1/membre/deseinscrireDeEvent?membreId=${idMembre}&eventId=${idEvenement}`)
            .then(() => {
              this.voirEvenements(idMembre);
            })
            .catch((error) => {
              console.error("Erreur lors de la désinscription du membre de l'événement", error);
            });
      } else {
        console.error("ID du membre ou de l'événement non défini lors de la désinscription");
      }
    },
